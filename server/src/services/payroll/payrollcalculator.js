...
const calculateProratedAmount = ({
  ...
  return roundMoney((value / workingDays) * paidDays);
};
...
export const calculatePayroll = ({
  ...
  if (!profile) {
    throw new Error("Payroll profile is required");
  }
  ...
  if (
    ...
    throw new Error("Paid days must be between 0 and working days");
  }
  ...
  const prorate = (amount) =>
    calculateProratedAmount({
      amount: Number(amount || 0) * monthlyMultiplier,
  ...
  const earnings = {
    ...
    specialAllowance: prorate(compensation.specialAllowance),
  ...
  const professionalTax =
    calculateProfessionalTax({
      ...
      applicable: profile.professionalTaxApplicable,
      rule: payrollRule.professionalTax,
    });

  const voluntaryPf = roundMoney(
    Number(profile.voluntaryPfContribution || 0)
  );

  const deductions = {
    employeePf: epf.employeeContribution,

    employeeEsi: esi.employeeContribution,

    professionalTax: professionalTax.tax,

    // TDS will be connected to PayrollYtd in the next step.
    tds: 0,
  ...
  const employerContributions = {
    employerPf: epf.employerContribution,
